...
class OdometryPlotter(Node):
    def __init__(self):
        ...
        # Matplotlib setup
        self.fig, self.axes = plt.subplots(3, 3, figsize=(12, 12))
    ...
    def update_plot(self):
        ...
        for i, (label, key, color) in enumerate(zip(labels, keys, colors)):
            ...
            # Angular Velocity Z
            self.axes[2][i].plot(self.data['ground_truth']['ang_z'], label='Ground Truth', linestyle='-', color='blue')
            self.axes[2][i].plot(self.data[key]['ang_z'], label=label, linestyle='-.', color=color)
            self.axes[2][i].set_title(f'Ground Truth vs {label} (Angular Velocity Z)')
            self.axes[2][i].set_xlabel('Time Step')
            self.axes[2][i].set_ylabel('Angular Velocity Z (rad/s)')
